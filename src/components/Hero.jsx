...
export default function Hero({ strings, onOpenExcerpt, onOpenPreorder, onOpenBookPreview }) {
  return (
    <section className="relative isolate overflow-hidden bg-white pt-28 sm:pt-36">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2">
          <div className="max-w-xl">
            <p className="text-xs uppercase tracking-widest text-gray-500">{strings.hero.brand}</p>
            <h1 className="mt-5 font-serif text-5xl italic leading-tight tracking-tight text-gray-900 sm:text-6xl md:text-7xl">{strings.hero.title}</h1>
            <p className="mt-6 text-base text-gray-600 sm:text-lg">{strings.hero.sub}</p>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              ...
              <button type="button" onClick={onOpenBookPreview} className="inline-flex items-center rounded-full border border-black/10 bg-white px-5 py-3 text-sm font-semibold text-gray-900 hover:bg-gray-50">{strings?.hero?.brand === 'The Leader’s Inner Forge' ? 'Preview book' : 'Prévisualiser le livre'}</button>
            </div>
          </div>
