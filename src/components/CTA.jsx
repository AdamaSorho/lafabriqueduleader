export default function CTA({ strings, onOpenExcerpt, onOpenPreorder, onOpenBookPreview }) {
  return (
    <section className="relative isolate mx-4 my-20 rounded-3xl bg-gray-900 px-6 py-14 text-white sm:mx-auto sm:max-w-7xl sm:px-12">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          ...
          <button type="button" onClick={onOpenBookPreview} className="inline-flex items-center rounded-full border border-white/20 px-5 py-3 text-sm font-semibold hover:bg-white/10">{strings?.hero?.brand === 'The Leader’s Inner Forge' ? 'Preview book' : 'Prévisualiser le livre'}</button>
        </div>
      </div>
